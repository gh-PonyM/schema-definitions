...
def validate_project_environment(settings: Settings, target: str) -> ProjectEnvironment:
    ...
    try:
        ...
        typer.secho("Error: Target must be in format 'project.environment'", err=True, fg=typer.colors.RED)
        raise typer.Exit(1)

    if project_name not in settings.projects:
        typer.secho(f"Error: Project '{project_name}' not found in settings", err=True, fg=typer.colors.RED)
        raise typer.Exit(1)

    project_config = settings.projects[project_name]
    if env_name not in project_config.db:
        typer.secho(f"Error: Environment '{env_name}' not found in project '{project_name}'", err=True, fg=typer.colors.RED)
        raise typer.Exit(1)
...
def validate_matching_db_types(src_db: DatabaseConfig, tgt_db: DatabaseConfig) -> None:
    ...
    if src_db.type != tgt_db.type:
        typer.secho(f"Error: Database types must match (source: {src_db.type}, target: {tgt_db.type})", err=True, fg=typer.colors.RED)
        raise typer.Exit(1)
